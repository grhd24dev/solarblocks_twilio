var onload_script = function () {
  ...
  frappe.run_serially([() => setup_device(), () => dialer_screen()]);
    ...
    frappe.call({
      ...
      callback: (data) => {
        ...
        device.on("ready", function (device) {
          Object.values(frappe.twilio_conn_dialog_map).forEach(
            function (popup) {
        ...
        device.on("disconnect", function (conn) {
          ...
          popup.hide_dialpad();
          popup.dialog.set_df_property("note", "hidden", 1)
          // Make sure that dialog is closed when incoming call is disconnected.
          if (conn.direction == "INCOMING") {
            popup.close();
          }
          console.log(conn, popup)
          frappe.call({
            method: "solarblocks_twilio.integrations.twilio.api.add_note_to_call_log",
            args: {
              call_sid: conn.parameters.CallSid,
              note: popup.dialog.fields_dict.note.value
            },
            callback: (r) => {
              console.log(r)
            }
        ...
        device.on("connect", function (conn) {
          ...
          popup.dialog.set_df_property("note", "hidden", 0)
          ...
          document.onkeydown = (e) => {
            if (popup.dialog.$wrapper.find(".dialpad-section").is(":hidden"))
              return;
            let key = e.key;
            if (
              conn.status() == "open" &&
  ...
  function call_screen(conn) {
    frappe.call({
      type: "GET",
      method: "solarblocks_twilio.integrations.twilio.api.get_contact_details",
      args: {
