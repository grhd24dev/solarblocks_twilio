...
class Twilio:
	...
	def get_recording_status_callback_url(self):
		url_path = "/api/method/solarblocks_twilio.integrations.twilio.api.update_recording_info"
		return get_public_url(url_path)

	def get_update_call_status_callback_url(self):
		url_path = "/api/method/solarblocks_twilio.integrations.twilio.api.update_call_status_info"
		return get_public_url(url_path)
...
def get_the_call_attender(owners, caller=None):
	...
	if len(current_loggedin_users) > 1 and caller:
		...
		if not deal_owner:
			deal_owner = frappe.db.get_value('Lead', {'mobile_no': caller}, 'lead_owner')
